...
uses
  ...
  StdCtrls, Buttons, messages, Mask, Zlib, Spin, Menus, Dialogs,
  XPMan;
...
procedure TForm1.LB1C(Sender: TObject);
var sel:integer;
begin
  timer1.interval:=0;
  button1.caption:='Rotation';
  sel:=Listbox1.itemindex;
  if sel<18 then RadioB3.caption:='In Ball'
            else RadioB3.caption:='Around Ball';
  if (sel<5) or (sel>17) then radiob2.caption:='In ball'
            else radiob2.caption:='inner sphere';
  b1c(sender);
end;
...
const
{ Solid ?, Cuboctahedron, Icosidodecahedron, Rhombicosidodecahedron,
  Rhombicuboctahedron, Beveled Hexahedron, Beveled Dodecahedron,
  Frustrated Icosahedron, Truncated Icosidodecahedron, Truncated Cuboctahedron, Truncated Hexahedron,
  Frustrated Octahedron, Truncated Tetrahedron, Truncated Dodecahedron, Pyramidal Tetrahedron,
  Pyramidal Octahedron, Pyramidal Icosahedron, Pyramidal Cube, Rhomboid Dodecahedron,
  24-sided Deltoid, 60-sided Deltoid, Rhomboid 30-sided Deltoid,
  Pyramidal Dodecahedron, 24-sided Pentagon, 60-sided Pentagon,
  Disdyakistriacontahedron, Disdyakisdodecahedron}
...
begin
  ...
  case k_pnr of
    0..4 : begin
              {The next polyhedron will be number 451 in poly1 and poly2.}
             kk:='['+inttostr(k_pnr+1)+']';
...
procedure tForm1.zeichnenx(canvas:tcanvas);
...
//front
    ...
    for i:=0 to janz-1 do
      if reihe[i]<0 then
        drawpoly4(reihenfolge[i],filelines3[reihenfolge[i],0],0);
    canvas.copyMode:=cmsrcand;
    canvas.draw(0,0,bitmap2);
    bitmap2.free;
  end;
...
procedure tForm1.drehen(x,y,z:double);
var i:integer;
    x0,y0:double;
...
procedure TForm1.T1Tim(Sender: TObject);
begin
...
procedure TForm1.PB1MD(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TForm1.PB1MM(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if live then begin
...
procedure TForm1.PB1MU(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TForm1.x2C(Sender: TObject);
begin
  b1c(sender);
end;

procedure TForm1.t6C(Sender: TObject);
begin
